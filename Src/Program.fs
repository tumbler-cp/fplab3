...
let computeDividedDifferences (points: Point list) : float[,] =
    ...
    let dd =
        ...
        |> List.fold
            (fun acc (i, y) ->
                Array2D.set acc i 0 y
                acc)
            (Array2D.zeroCreate<float> n n)
            ...
            |> List.fold
                (fun acc i ->
                    Array2D.set
                        ...
                        i
                        j
                        ((Array2D.get acc (i + 1) (j - 1) - Array2D.get acc i (j - 1))
            ...
            | "-n" ->
                if i + 1 < args.Length then
                    ...
                    aux
                        (i + 2)
                        { opts with
                            newtonWindowSize = nVal
                            useNewton = true }
                else
                    failwith "! Enter value for -n"
            | "-l" ->
                if i + 1 < args.Length then
                    ...
                    aux
                        (i + 2)
                        { opts with
                            lagrangeWindowSize = lVal
                            useLagrange = true }
                else
                    failwith "! Enter value for -l"
            | _ -> aux (i + 1) opts
        ...
        | Some(point) ->
            let linearState =
                if opts.useLinear then
                    processLinear opts.step point linearState
                else
                    linearState

            let newtonState =
                if opts.useNewton then
                    processNewton opts.step point newtonState
                else
                    newtonState

            let lagrangeState =
                if opts.useLagrange then
                    processLagrange opts.step point lagrangeState
                else
                    lagrangeState
...
let processLines (opts: Options) =
    ...
    let rec loop linearState newtonState lagrangeState lastPoint =
        ...
        | -1 ->
            ...
            | Some(p) ->
                if opts.useLinear then
                    flushLinear opts.step linearState p

                if opts.useNewton then
                    flushNewton opts.step newtonState

                if opts.useLagrange then
                    flushLagrange opts.step lagrangeState
            | None -> ()
        | _ ->
            let line = Console.In.ReadLine()

            let (linearState, newtonState, lagrangeState, lastPoint) =
                processLine opts line linearState newtonState lagrangeState lastPoint
...
let main argv =
    ...
    | false, false, false ->
        eprintfn
            "! Method is not chosen --linear (-l for ), --newton (-n for interpolation point count), --lagrange, (--step for interpolation step)"
